...
const rows=[
{id:1,name:'Sample Resident One',national_id:'A000001',house:'Sample House A',phone:'7000001',party:'PNC',assignStatus:'unassigned',assigner:'',callStatus:'not called',voteStatus:'not voted',visitStatus:'not visited',visitStaff:'Ahmed Mohamed',visitNotes:'Survey not started',transportStatus:'none'},
{id:2,name:'Sample Resident Two',national_id:'A000002',house:'Sample House B',phone:'7000002',party:'MDP',assignStatus:'assigned',assigner:'Team A',callStatus:'connected',voteStatus:'voted',visitStatus:'visited',visitStaff:'Saeed Rashid',visitNotes:'Survey completed',transportStatus:'confirmed'},
{id:3,name:'Sample Resident Three',national_id:'A000003',house:'Dhafthar',phone:'7000003',party:'PNC',assignStatus:'completed',assigner:'Team B',callStatus:'follow-up',voteStatus:'mail-in',visitStatus:'scheduled',visitStaff:'Team B',visitNotes:'Resident not home. Follow-up scheduled.',transportStatus:'need transport'}
];
...
visits:{title:'Door to Door',subtitle:'Track door-to-door visits',stats:['Total Households','Visited','Not Visited','Scheduled'],filters:['address','visitStatus'],columns:[]},
...
const statusClass=value=>'status-'+String(value||'').toLowerCase().replace(/\s+/g,'-');
...
function filtered(section){
 ...
 if(state.search){const q=state.search.toLowerCase();data=data.filter(r=>[r.name,r.national_id,r.phone,r.house].join(' ').toLowerCase().includes(q));}
...
function filters(section){
 ...
 if(list.includes('visitStatus'))html+='<label>Visit Status<select data-filter="visitStatus"><option value="all">All Status</option><option value="visited">Visited</option><option value="not visited">Not Visited</option><option value="scheduled">Scheduled</option></select></label>';
...
function visitCards(data){
 ...
 return`<div class="section-toolbar"><h2>Door to Door Visits</h2><button class="btn primary">Add Visit</button></div>${filters('visits')}<div class="cards-grid">${visible.map(row=>`<article class="record-card"><div class="card-header"><h4>${esc(row.name)}</h4><span class="status ${statusClass(row.visitStatus)}">${esc(row.visitStatus)}</span></div><div class="field"><span class="label">Staff</span><span class="value">${esc(row.visitStaff||'Unassigned')}</span></div><div class="field"><span class="label">Resident</span><span class="value">${esc(row.name)}</span></div><div class="field"><span class="label">Address</span><span class="value">${esc(row.house)}</span></div><div class="field"><span class="label">Status</span><span class="value">${esc(row.visitStatus)}</span></div><div class="field"><span class="label">Notes</span><span class="value">${esc(row.visitNotes||'-')}</span></div><div class="card-actions"><button class="btn edit">Edit</button><button class="btn delete">Delete</button></div></article>`).join('')||'<div class="empty-card">No visits found</div>'}</div><div class="load-row"><button class="btn secondary" id="loadMore" ${visible.length>=data.length?'disabled':''}>Load More</button><span>Showing ${visible.length} of ${data.length}</span></div>`;
}
...
function render(){document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.section===state.section));document.getElementById('globalParty').value=state.party;if(state.section==='dashboard')return dashboard();const data=filtered(state.section);const body=state.section==='visits'?visitCards(data):`${filters(state.section)}${table(state.section,data)}`;document.getElementById('pageContent').innerHTML=`<section class="page-head"><div><p class="eyebrow">${esc(config[state.section].title)}</p><h1>${esc(config[state.section].title)}</h1><p>${esc(config[state.section].subtitle)}</p></div><span class="record-pill">${data.length} records</span></section>${stats(state.section,data)}<section class="panel">${body}</section>`;}
