(() => {
  ...
  const rowsHtml = payload.rows.map(row => `<tr>
    <td>${esc(row.id)}</td>
    <td>${row.photo ? `<img src="${esc(row.photo)}" alt="${esc(row.name || 'Resident')}">` : '-'}</td>
  ...
  <div class="preview-table-wrap"><table class="preview-table"><thead><tr><th>ID</th><th>Photo</th><th>ID Number</th><th>Name</th><th>Official Address</th><th>Living Now</th><th>Mobile</th><th>Sex</th><th>Age</th><th>Campaign Vote</th><th>Turnout</th></tr></thead><tbody>${rowsHtml}</tbody></table></div>
